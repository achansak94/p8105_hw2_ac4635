...
```{r Transit_Details, collapse=TRUE}
...
There are `r count(distinct(NYC_transit_df, line, station_name, na.rm=TRUE))` distinct stations.

There are `r count(filter(NYC_transit_df, ada == TRUE, na.rm=TRUE) %>% distinct(line, station_name))` stations that are ADA compliant. 

Among the `r count(filter (NYC_transit_df, vending == "NO", na.rm=TRUE))` subway entrances/exits without vending, `r count(filter(NYC_transit_df, vending == "NO", na.rm=TRUE) %>% filter (entry == "TRUE"))` allowed entry. 
